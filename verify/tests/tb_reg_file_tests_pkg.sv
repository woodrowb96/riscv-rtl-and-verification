package tb_reg_file_tests_pkg;
  ...
  import tb_reg_file_predictor_pkg::*;

  /*=============================== BASE TEST ==================================*/
  virtual class reg_file_base_test #(type GEN_T) extends base_test #(
      reg_file_trans, GEN_T, reg_file_driver, reg_file_monitor, reg_file_predictor, reg_file_scoreboard);
    ...
    protected function new(virtual reg_file_intf vif, tb_reg_file_coverage coverage, string tag = "REG_FILE_BASE_TEST");
      ...
      pred = new(vif, "REG_FILE_PRED", pred_to_scb_mbx);
      scb  = new(coverage, "REG_FILE_SCB", mon_to_scb_mbx, pred_to_scb_mbx);
    endfunction
  endclass
