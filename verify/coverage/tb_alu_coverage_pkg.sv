package tb_alu_coverage_pkg;
  ...
  class alu_coverage;
    ...
    function void sample(alu_trans trans);
      this.trans = trans;
      cg.sample();
    endfunction
    ...
    covergroup cg;
      ...
      alu_op: coverpoint trans.alu_op {
        bins op_and = {ALU_AND};
        bins op_or  = {ALU_OR};
        bins op_add = {ALU_ADD};
        bins op_sub = {ALU_SUB};
      ...
      /************** ZERO FLAG  COVERAGE *******************/
      zero_flag: coverpoint trans.zero{
        bins set   = {1'b1};
        bins unset = {1'b0};
      }
      ...
      in_a_log_op: coverpoint trans.in_a
        iff (trans.alu_op inside {ALU_AND, ALU_OR}) {
      ...
      in_b_log_op: coverpoint trans.in_b
        iff (trans.alu_op inside {ALU_AND, ALU_OR}) {
          bins all_zero    = {WORD_ALL_ZEROS};
          bins alt_ones_55 = {WORD_ALT_ONES_55};
          bins alt_ones_aa = {WORD_ALT_ONES_AA};
      ...
      in_b_add_op: coverpoint trans.in_b
        iff (trans.alu_op == ALU_ADD) {
          bins zero             = {WORD_UNSIGNED_ZERO};
          bins one              = {WORD_UNSIGNED_ONE};
          bins max_unsigned     = {WORD_MAX_UNSIGNED};
      ...
      add_overflow: coverpoint detect_add_overflow(trans.in_a, trans.in_b) 
        iff (trans.alu_op == ALU_ADD){
            bins yes = {1};
            bins no = {0};
        }
      ...
      //we want to hit the following corners values/ranges for both inputs
      in_a_sub_op: coverpoint trans.in_a
        iff (trans.alu_op == ALU_SUB) {
      ...
      in_b_sub_op: coverpoint trans.in_b
        iff (trans.alu_op == ALU_SUB) {
          bins zero                 = {WORD_SIGNED_ZERO};
          bins signed_pos_one       = {WORD_SIGNED_POS_ONE};
          bins signed_neg_one       = {WORD_SIGNED_NEG_ONE};
      ...
      sub_overflow: coverpoint detect_sub_overflow(trans.in_a, trans.in_b)
        iff (trans.alu_op == ALU_SUB){
            bins yes = {1};
            bins no = {0};
        }
